...
while true; do
    if [ $(sh $FUSE_SERVER_BIN/client 'profile-list'| grep "demo-websocketbindydemo"| wc -l ) -ge 1 ]; then
        break
    fi
    sleep 2
done
...
echo         
sh $FUSE_SERVER_BIN/client -r 2 -d 40 'container-create-child  --profile demo-websocketbindydemo root wsbindycon'
